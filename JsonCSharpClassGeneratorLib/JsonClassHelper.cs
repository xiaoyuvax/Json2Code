...
namespace JsonCSharpClassGenerator
{
    internal static class JsonClassHelper
    {
        ...
        {
            if (obj.TryGetValue(field, out JToken value)) return GetJToken<T>(value);
            else return null;
        }
        ...
        {
            var value = GetJToken<JValue>(token) ?? throw new Newtonsoft.Json.JsonSerializationException();
